...
제가 사용한 핵심기술인 미디어파이프는 구글이 개발한 오픈소스 ai 프레임워크로
미리 학습된 모델을 임포트하여 탑재함

html에 캔버스로 손과의 상호작용 표현함


미디어파이프의 모델중 핸드랜드마커 사용함
이 기능은 실시간으로 비디오에서 손을 찾아내고 21관절의 위치를 좌표 반환함
좌표 데이터를 받아 4엄지끝, 8검지끝 좌표 거리로 집게 손동작을 감지함
ㄴ더자세히
4,8의 정규화 좌표를 픽셀 좌표로 변환
좌표사이 거리 계산
특정 픽셀보다 가까워지면 집게모양으로 판단

(문제해결과정)
프레임마다 입력 받아서인지 잠깐 끊기면 바로 드래그가 멈춤 손과 떨어짐
그래서 잡은상태에서 프레임이 잠깐 끊겨도 손가락이 안떨어져있으면 강제로 상태 유지
의도하지 않은 현상 방지로 안정적인 조작

카메라와 손과의 거리는 따로 z값을 사용하지 않고 손의 크기(손목과 중지뿌리0,9 픽셀 거리로)계산
